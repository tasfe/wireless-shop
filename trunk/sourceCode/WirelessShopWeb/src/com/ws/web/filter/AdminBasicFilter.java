...
public class AdminBasicFilter extends WRBasicFilter{
	
	public static final String REGION = "admin";
	public static final String LOGIN_USER_SESSION_KEY = "admin_login_user_session_key";
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
		...
		if(isJScript(type) || isPicture(type)){
			...
			AdminPageFilter.doFilter(request, response, chain, target);
		}else if(isAction(type)){
			response.setContentType("text/plain; charset=utf-8");
			AdminActionFilter.doFilter(request, response, chain, target);
		}else{
			try{
				response.setContentType("text/plain; charset=utf-8");
