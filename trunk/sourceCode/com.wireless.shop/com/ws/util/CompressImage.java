...
public final class CompressImage {
    ...
    private int width, height, scaleWidth;
    double support = (double) 3.0;
    double PI = (double) 3.14159265358978;
    double[] contrib, normContrib, tmpContrib;
    int startContrib, stopContrib, nDots, nHalfDots;
    ...
    public void saveImageAsJpg(String fromFileStr, String saveToFileStr,
        ...
        int[] size = calcSize(srcImage, formatWideth, formatHeight);
        srcImage = imageZoomOut(srcImage, (int)(size[1] * 1.618), (int)size[1]);
     ...
     * @return
     */
    private int[] calcSize(BufferedImage srcImage, int formatWideth, int formatHeight){
        ...
        if (imageWideth > 0 && imageHeight > 0) {
            if (imageWideth / imageHeight >= formatWideth / formatHeight) {
                if (imageWideth > formatWideth) {
                    ...
                    changeToHeight = (imageHeight * formatWideth) / imageWideth + 0.1;
                } else {
                ...
                if (imageHeight > formatHeight) {
                    ...
                    changeToWideth = (imageWideth * formatHeight) / imageHeight + 0.1;
                } else {
                    changeToWideth = imageWideth;
                    changeToHeight = imageHeight;
                }
        ...
        return new int[]{(int) changeToWideth, (int) changeToHeight};
    }
     ...
     * @throws IOException
     */
    public InputStream getSmallStream(File src, String type, int formatWideth, int formatHeight) throws IOException{
    	// 生成缩略图并上传至oss
		BufferedImage srcImg = javax.imageio.ImageIO.read(src);
		int[] size = calcSize(srcImg, formatWideth, formatHeight);
		BufferedImage smallImg = imageZoomOut(srcImg, size[0], size[1]);
		
		ByteArrayOutputStream bs = new ByteArrayOutputStream();
		ImageOutputStream ios = ImageIO.createImageOutputStream(bs);
		ImageIO.write(smallImg, type, ios);
